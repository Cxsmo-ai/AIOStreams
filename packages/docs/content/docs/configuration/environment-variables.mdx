...
| `BUILTIN_DOWNLOAD_POLL_INTERVAL`                | Debrid › Download Poll Interval         | map      | `{"nzbdav":2000,"altmount":2000,"stremthru_newz":2000,"*":10000,"torbox":5000}`      | Per-service download-status poll interval. Env shape: `service:duration,...`. Wildcard `*` covers unlisted services. |
| `BUILTIN_DOWNLOAD_MAX_WAIT_TIME`                | Debrid › Download Max Wait Time         | map      | `{"nzbdav":90000,"altmount":90000,"stremthru_newz":90000,"*":120000,"torbox":90000}` | Per-service maximum wait time before timing out a download check. Env shape: `service:duration,...`.                 |
...
| Environment Variable                | UI Setting                  | Type     | Default   | Description                                       |
| ----------------------------------- | --------------------------- | -------- | --------- | ------------------------------------------------- |
| `BUILTIN_HARBRR_URL`                | Harbrr › URL                | string   | —         | Base URL for the Harbrr built-in addon.           |
| `BUILTIN_HARBRR_API_KEY`            | Harbrr › API Key            | string   | _(unset)_ | API key for the Harbrr built-in addon. _(secret)_ |
| `BUILTIN_HARBRR_INDEXERS`           | Harbrr › Indexers           | list     | —         | Comma-separated list of Harbrr indexers to query. |
| `BUILTIN_DEFAULT_HARBRR_TIMEOUT`    | Harbrr › Default Timeout    | number   | —         | Default timeout for Harbrr requests.              |
| `BUILTIN_HARBRR_SEARCH_TIMEOUT`     | Harbrr › Search Timeout     | number   | `30000`   | Timeout for Harbrr search requests.               |
| `BUILTIN_HARBRR_SEARCH_CACHE_TTL`   | Harbrr › Search Cache TTL   | duration | `604800`  | Cache TTL for Harbrr search results.              |
| `BUILTIN_HARBRR_INDEXERS_CACHE_TTL` | Harbrr › Indexers Cache TTL | duration | `1209600` | Cache TTL for the Harbrr indexers list.           |
