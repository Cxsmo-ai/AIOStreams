...
function isIntegrationService(id: string): boolean {
  return INTEGRATION_SERVICE_IDS.includes(id);
}
/**
 ...
 */
const SERVICE_LOGO_MAP: Partial<Record<ServiceId, string>> = {
...
export function StreamServices() {
  ...
  const filteredServices = (userData.services ?? []).filter((service) => {
    ...
    if (categoryFilter !== 'all') {
      ...
      const integration = isIntegrationService(service.id);
      if (!dual) {
        if (categoryFilter === 'integration' && !integration) return false;
        if (categoryFilter === 'debrid' && (usenet || integration))
          return false;
        if (categoryFilter === 'usenet' && (!usenet || integration))
          return false;
      }
    }
    if (searchQuery.trim()) {
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
        ...
        <div className="flex gap-1.5 shrink-0">
          {(['all', 'debrid', 'usenet', 'integration'] as const).map((cat) => {
            const activeClass =
              cat === 'debrid'
